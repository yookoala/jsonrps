...
// testLogWriter wraps testing.T.Log to implement io.Writer
...
// newTestLogger creates a logger that outputs to t.Log
...
func TestSession_WriteHeader(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: "RPS/1.0 404 Not Found\r\nContent-Type: application/json\r\nServer: test-server/1.0\r\n\r\n",
		},
		{
			name:       "500 Internal Server Error",
			statusCode: 500,
			headers:    http.Header{},
			expected:   "RPS/1.0 500 Internal Server Error\r\n\r\n",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			session.WriteResponseHeader(tt.statusCode)
			...
			// Check status line
			expectedStatusLine := fmt.Sprintf("RPS/1.0 %d %s", tt.statusCode, http.StatusText(tt.statusCode))
...
func TestSession_Write_WithHeadersAlreadySent(t *testing.T) {
	...
	// Send headers first
	session.WriteResponseHeader(200)
...
func TestSession_WriteHeader_And_Write_Integration(t *testing.T) {
	...
	if !strings.Contains(written, "RPS/1.0 200 OK") {
...
func TestSession_WriteHeader_Multiple_Calls(t *testing.T) {
	...
	session.WriteResponseHeader(404)
...
func TestSession_WriteRequestHeader(t *testing.T) {
	tests := []struct {
		name         string
		method       string
		localHeaders http.Header
		...
		{
			name:   "basic request header with GET method",
			method: "GET",
			localHeaders: http.Header{
				"Content-Type": []string{"application/json+rps"},
				"User-Agent":   []string{"test-client/1.0"},
			},
			wantProtocol: "RPS/1.0 GET",
			wantHeaders: []string{
				"Content-Type: application/json+rps",
				"User-Agent: test-client/1.0",
			},
		},
		{
			name:   "POST method with authorization header",
			method: "POST",
			localHeaders: http.Header{
				"Content-Type":  []string{"application/json+rps"},
				"Authorization": []string{"Bearer abc123"},
			},
			wantProtocol: "RPS/1.0 POST",
			wantHeaders: []string{
		...
		{
			name:   "custom method with multiple header values",
			method: "CUSTOM",
			localHeaders: http.Header{
				"Accept": []string{"application/json", "text/plain"},
				"X-Test": []string{"value1"},
			},
			wantProtocol: "RPS/1.0 CUSTOM",
			wantHeaders: []string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create a no-op logger to avoid nil pointer dereference
			logger := newTestLogger(t)

			// Create session with test data
			session := &jsonrps.Session{
				ProtocolSignature: jsonrps.DefaultProtocolSignature,
				LocalHeaders:      tt.localHeaders,
				Conn:              conn,
				Logger:            logger,
			}

			// Call the method under test
			session.WriteRequestHeader(tt.method)

			// Get the written data
			actualOutput := conn.writeData.String()

			// Verify the protocol line is correct
			if !strings.HasPrefix(actualOutput, tt.wantProtocol+"\r\n") {
				t.Errorf("WriteRequestHeader() should start with %q, got: %q", tt.wantProtocol+"\r\n", actualOutput)
			}

			// Verify the output ends with double CRLF
			if !strings.HasSuffix(actualOutput, "\r\n\r\n") {
				t.Errorf("WriteRequestHeader() should end with double CRLF, got: %q", actualOutput)
			}

			// Verify all expected headers are present (order may vary due to map iteration)
			for _, expectedHeader := range tt.wantHeaders {
				if !strings.Contains(actualOutput, expectedHeader+"\r\n") {
					t.Errorf("WriteRequestHeader() output missing expected header: %q\nactual: %q", expectedHeader, actualOutput)
				}
			}

			// Count header lines to ensure no extra headers
			lines := strings.Split(actualOutput, "\r\n")
			// Lines: [0]=protocol, [1..n-2]=headers, [n-1]="", [n]=""
			actualHeaderCount := len(lines) - 3 // subtract protocol line and two empty lines
			expectedHeaderCount := len(tt.wantHeaders)
			if actualHeaderCount != expectedHeaderCount {
				t.Errorf("WriteRequestHeader() expected %d header lines, got %d. Lines: %v", expectedHeaderCount, actualHeaderCount, lines)
			}
		})
	}
}

func TestSession_WriteRequestHeader_WithCustomProtocolSignature(t *testing.T) {
	conn := &mockReadWriteCloser{}
	...
	session := &jsonrps.Session{
		ProtocolSignature: "CUSTOM/2.0", // This doesn't affect WriteRequestHeader since it uses DefaultProtocolSignature
		LocalHeaders: http.Header{
			"Content-Type": []string{"application/json"},
		},
		Conn:   conn,
	...
	session.WriteRequestHeader("GET")

	// WriteRequestHeader always uses DefaultProtocolSignature, not session.ProtocolSignature
	expectedOutput := "RPS/1.0 GET\r\nContent-Type: application/json\r\n\r\n"
	actualOutput := conn.writeData.String()

	if actualOutput != expectedOutput {
		t.Errorf("WriteRequestHeader() should use DefaultProtocolSignature:\nexpected: %q\nactual:   %q", expectedOutput, actualOutput)
	}
}
